...
if test_on_test_dataset:
    test_batch_size = 2933
else:
    test_batch_size = 2961
test_batch_size = 100
...
for epoch in range(1, nb_epochs+1):
    ...
    if not(epoch % 100): 
        score = my_test() 
        print("test mean accuracy %f : current epoch %d/%d" % (score,epoch,nb_epochs))
        score_list.append(score)
        epoch_list.append(epoch)


scores_file = open('%s_cub_scores.txt' % test_dataset_string,'w')
for iscore in range(len(score_list)):
    scores_file.write("epoch %f : %d\n" % (score_list[iscore],epoch_list[iscore]))
scores_file.close()
